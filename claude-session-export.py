...
Runs as a SessionEnd hook (step 1 of 2). Parses the session transcript JSONL,
extracts structured metadata, and writes a markdown note to a staging directory
(~/.local/share/claude-sessions/). A follow-up agent hook generates a summary
and edits it into the same file. obsidian-tools then syncs from the staging
directory into the Obsidian vault.

Writes temp files for the agent hook:
  /tmp/claude-session-export-note-path.txt  — path to the staged note
  /tmp/claude-session-export-transcript.md  — condensed human-readable transcript
...
MIN_MESSAGES = 2
TMP_NOTE_PATH = Path("/tmp/claude-session-export-note-path.txt")
TMP_TRANSCRIPT = Path("/tmp/claude-session-export-transcript.md")
...
def main() -> None:
    ...
    # Write temp files for the agent hook
    TMP_NOTE_PATH.write_text(str(filepath))
    transcript_text = build_condensed_transcript(entries)
    TMP_TRANSCRIPT.write_text(transcript_text)

    print(f"Exported session {session_id[:8]} → {filepath}", file=sys.stderr)
    print(
        f"Wrote condensed transcript to {TMP_TRANSCRIPT} for agent hook",
        file=sys.stderr,
    )
